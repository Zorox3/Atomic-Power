...
import de.atomicpower.tileEntities.TileEntityAtomicGeneratorFloor;

public class AtomicGeneratorFloor extends AtomBlockContainer {
			...
			for (int z2 = 0; z2 < structureSize; z2++) {
				if (!mStructure) {
							...
							if (currentCheckStructure
									&& !world
											...
											.equals(AtomicPower.blockRegistry
													.getBlockContainerByName("atomicGeneratorFloor"))) {
					...
					if (currentCheckStructure) {
						for (int x3 = 0; x3 < structureSize; x3++) {
							for (int z3 = 0; z3 < structureSize; z3++) {
								world.setBlockMetadataWithNotify(x + x2 - x3, y, z + z2 - z3, x3 * structureSize + z3 + 1, 2);
							}
						}
					}
				}
	...
	public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
		return new TileEntityAtomicGeneratorFloor(TileEntityAtomicGeneratorFloor.class, "atomicGeneratorFloorTile");
	}
